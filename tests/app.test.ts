...
describe('app', () => {
  ...
  describe('GET /api/jobs/:id events', () => {
    ...
    it('carries the search narration past the transcript spam that outnumbers it', async () => {
      ...
      ctx.events.append({ kind: 'agent.stop', jobId, message: '[acg.rip] round 1/1 (curl): gave up (nothing found): nothing yet' });
      const app = createApp(ctx);

      const detail: any = await (await app.request(`/api/jobs/${jobId}`)).json();

      const kinds = detail.events.map((e: { kind: string }) => e.kind);
      expect(kinds.filter((k: string) => k === 'agent.stop')).toHaveLength(1);
