...
import { ingestFixture } from './helpers.js';
...
const REAL_HANDLERS = { acquire: runAcquireJob, ingest: runIngestJob };
...
describe('end-to-end: webhook -> queue -> runner -> ingest', () => {
  ...
  it('a Download webhook enqueues an ingest job that the real runner (real handler map) picks up and runs, placing a sidecar', async () => {
    ...
    // A sidecar sitting in the torrent folder for the sweep to find and place.
    ...
    expect(fx.ctx.queue.list()).toMatchObject([{ status: 'done' }]);
