...
| 4 | Subtitle confidence and timing drift per match, a floor, Held / Applied / Rejected | Host drift is recorded on `placed_files` (`offsetMs`, `drift`) and the gate is automatic at hardcoded `acceptRatio` 0.80. The sidecar matcher is still yes/no. There is no Held / Applied / Rejected review UI and no Settings knob | A score on the review item, and a floor you can tune. That is the one that would make Needs review honest |
