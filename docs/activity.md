...
The trace unit is per job (`jobs.id`). Within a job, steps are sequential. Agent steps are keyed by the action the model chose (`agent.search`, `agent.download`, `agent.escalate`, …). The chosen action is written before the destination guard runs, so a blocked action is the `agent.refused` entry that follows it, not a different kind. `GET /api/traces/:jobId` carries the job's LLM cost: `calls` counts `llm.call` rows, and the token sums are over `llm.attempt` payloads.

The debug page is a superset of the Activity drawer: the target list on the left is the Activity list with each target's runs expanded; the Story tab is the drawer's run body. Above the trace, a lane strip draws the run by actor (trigger, arr, agent, llm, media, writes, verdicts) on a time axis that compresses idle gaps over two seconds. Its title row sums the run's active and idle time, and each lane label carries its own count. The strip zooms on scroll and pans on drag, up to 500x, with the axis ticks respaced for the visible window. Hovering a bar rings it and shows kind, summary and duration in a tooltip; clicking it selects the step. The waterfall below is the trace tree with an `at · took` column. Selecting a step opens the inspector; an `llm.call` shows its prompt, output, usage and the agent step its decision caused. The debug page never triggers or mutates anything.
